...
config_name = "config_regressor_dclaw"
...
def run(config: DictConfig) -> None:
    ...
    tb_logger = TensorBoardLogger(
        version  = '[{}]-[{}]-[dim_out={}]-[dim_fc_hidden={}]-[{}epoch]-[num_batch={}]-[beta={}]-[{}]-[{}]-{}'.format(
            config.model.name,
            config.datamodule.name,
            config.model.network.dim_out,
            config.model.network.ensemble_network.dim_hidden,
            config.trainer.max_epochs,
            config.datamodule.config_dataloader.train.batch_size,
            config.loss.beta,
