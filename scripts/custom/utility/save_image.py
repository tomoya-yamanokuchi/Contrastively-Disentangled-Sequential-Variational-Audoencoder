...
def save_image(save_path: str, image: Tensor, normalize: bool = True):
    num_batch, step, channel, width, height = image.shape
    image_list = []
    for i in range(num_batch):
        image_list.append(utils.make_grid(image[i], nrow=step, padding=2, pad_value=0.0, normalize=normalize))
